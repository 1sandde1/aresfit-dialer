...
assert(index.includes('aresfit-dialer-sandde-v2.html?v=20260726-retry-override-r1'), 'the repository entry link must cache-bust to the verified build');
...
assert(html.includes("const APP_BUILD = '2026.07.26'"), 'the visible build identifier is missing');
assert(!/setTimeout\([^\n]*(?:nl-biz|lead-search|time-in)[^\n]*focus/.test(html), 'delayed modal autofocus can steal user input');
assert(
  html.includes("const ROLLBACK_BASE_COMMIT = '7b9ed090b6b1ed6aa4adbcd8479a3f42ce4074f2'"),
